...
using FracturedTruth.Vanilla.Modules;
        ...
        switch (rpcType)
        {
            case CustomRPC.VersionCheck:
                try
                {
                    ...
                    PlayerVersion.playerVersion[__instance.PlayerId] = new PlayerVersion(version, tag, forkId);
                    ...
                    if (Main.VersionCheat.Value && AmongUsClient.Instance.AmHost)
                        PlayerVersion.playerVersion[__instance.PlayerId] = PlayerVersion.playerVersion[0];
...
internal static class RPC
{
    ...
    public static async void RpcVersionCheck()
    {
        ...
        if (!AmongUsClient.Instance.AmHost && PlayerVersion.playerVersion.TryGetValue(0, out var ver) && Main.ForkId != ver.forkId) return;

        while (PlayerControl.LocalPlayer == null) await Task.Delay(500);
        if (GameStates.ModHost && !Main.VersionCheat.Value)
        {
            ...
            writer.Write(cheating ? PlayerVersion.playerVersion[0].version.ToString() : Main.PluginVersion);
            writer.Write(cheating ? PlayerVersion.playerVersion[0].tag : $"{ThisAssembly.Git.Commit}({ThisAssembly.Git.Branch})");
            writer.Write(cheating ? PlayerVersion.playerVersion[0].forkId : Main.ForkId);
            writer.EndMessage();
        }
        PlayerVersion.playerVersion[PlayerControl.LocalPlayer.PlayerId] = new PlayerVersion(Main.PluginVersion, $"{ThisAssembly.Git.Commit}({ThisAssembly.Git.Branch})", Main.ForkId);
