...
class AmongUsClientEndGamePatch
{
    ...
    {
        SummaryText = new();
        foreach (var id in VanillaPlayerData.AllPlayerData.Keys)
...
class SetEverythingUpPatch
{
    ...
    public static void Postfix(EndGameManager __instance)
    {
        ...
        foreach (var kvp in VanillaPlayerData.AllPlayerData.Where(x => x.Value.IsImpostor != DidHumansWin))
        {
            var id = kvp.Key;
            var data = kvp.Value;
        ...
        VanillaPlayerData.AllPlayerData.Values.ToArray().Do(data => data.Dispose());
    }
}
