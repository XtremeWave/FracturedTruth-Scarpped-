...
public static class Utils
{
    ...
    public static string GetRoleInfoForVanilla(this RoleTypes role, bool InfoLong = false)
    {
        ...
        if (!GameStates.IsNormalGame) text = "HnS" + text;
    ...
    {
        string f = $"{Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory)}/FracturedTruth-logs/";
        string t = DateTime.Now.ToString("yyyy-MM-dd_HH.mm.ss");
        string filename = $"{f}FracturedTruth-v{Main.DisplayedVersion}-{t}.log";
        if (!Directory.Exists(f)) Directory.CreateDirectory(f);
        FileInfo file = new(@$"{Environment.CurrentDirectory}/BepInEx/LogOutput.log");
        file.CopyTo(@filename);
        if (PlayerControl.LocalPlayer != null)
        {
            if (popup) //PlayerControl.LocalPlayer.ShowPopUp(string.Format(GetString("Message.DumpfileSaved"), $"FracturedTruth - v{Main.DisplayedVersion}-{t}.log"));
                HudManager.Instance.ShowPopUp(string.Format(GetString("Message.DumpfileSaved"), $"FracturedTruth - v{Main.DisplayedVersion}-{t}.log"));
            else AddChatMessage(string.Format(GetString("Message.DumpfileSaved"), $"FracturedTruth - v{Main.DisplayedVersion}-{t}.log"));
        }
    ...
    public static string SummaryTexts(byte id)
    {
        ...
        var thisdata = VanillaPlayerData.GetPlayerDataById(id);

        var builder = new StringBuilder();
        var longestNameByteCount = VanillaPlayerData.GetLongestNameByteCount();
    ...
    {
        var path = PathHelper.imageDownloader_savepath + file;
    ...
    public static PlayerControl GetPlayerById(byte playerId)
    {
        ...
        if (player == null) player = VanillaPlayerData.GetPlayerById(playerId);
    ...
    {
        var data = VanillaPlayerData.GetPlayerDataById(playerId);
    ...
    public static string GetVitalText(byte playerId,  bool summary = false)
    {
        ...
        if (data.IsDead && data.RealDeathReason == VanillaDeathReason.None)
            data.SetDeathReason(VanillaDeathReason.Exile);//WarpUp未执行时的处理
    ...
    {
        if (!GameStates.IsNormalGame) return false;
