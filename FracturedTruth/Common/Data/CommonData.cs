...
namespace FracturedTruth.Common.Data;
...
x.OwnerId == AmongUsClient.Instance.HostId && x.ModClient());
    public static bool IsLobby => AmongUsClient.Instance.GameState == InnerNet.InnerNetClient.GameStates.Joined && !IsFreePlay;
...
static class FracturedAllDataMethods
{
    public static VanillaPlayerData GetPlayerData(this PlayerControl pc) => VanillaPlayerData.GetPlayerDataById(pc.PlayerId) ?? null;
    public static bool IsAlive(this PlayerControl pc) => GameStates.IsLobby || pc?.GetPlayerData()?.IsDead == false;
    public static string GetDataName(this PlayerControl pc) => VanillaPlayerData.GetPlayerNameById(pc.PlayerId);
